...
if __name__ == "__main__":
    ...
    ds = AST_SpeechQualityDataset(df, data_dir)
    ...
    epoch = checkpoint['epoch'] # don't need this, remove
    loss = checkpoint['loss'] # don't need this, remove
    model.to(device)
    model.eval()

    print(f"\nModel loaded from epoch {epoch} with validation loss {loss:.6f}") # don't need this, remove
    ...
    with torch.no_grad():  # Disable gradient tracking for inference
        ...
        for b, (index, batch_features, batch_labels) in enumerate(dl):
